...
### 2.5. Кроссплатформенная независимость от внешних линкеров
* Все ключевые оптимизации (`opt-level = "z"`, `lto = true`, `codegen-units = 1`, `panic = "abort"`, `strip = true`) реализованы **на уровне LLVM и компилятора `rustc`**.
* Это исключает зависимость от специфических флагов линкеров разных ОС (GNU `ld` на Linux, Apple `ld64` на macOS, `lld-link` на Windows, `musl-cross ld` для MIPS/ARM) и гарантирует надежную сборку на всех архитектурах без поломки кросс-компиляции.
