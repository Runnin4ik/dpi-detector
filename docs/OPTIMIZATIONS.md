...
### 2.9. The Clock in the Report File Name (`time`)
...
* **Files:** `Cargo.toml` (`[workspace.dependencies]`, `[patch.crates-io]`, `exclude`), `crates/dpi-core/Cargo.toml`, `crates/dpi-core/src/net/http.rs`, `crates/dpi-core/src/net/fingerprint/h2.rs`, `crates/dpi-core/src/dns/doh.rs`, `vendor/h2/` (removed)
...
3. **Keeping DoH HTTP/2 (`http2`):**
   * The HTTP/2 stack is kept for authentic emulation of modern browser requests to DoH resolvers (Cloudflare, Google, AdGuard, Quad9). It is the published `http2` crate — h2's own source with the request-shape knobs as API — not a fork of our own: the `h2` patch this tree used to carry is gone (§2.10).
