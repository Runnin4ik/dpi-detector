...
| `chrome123` | `curl_chrome123` | Chrome 119–123: без padding, ALPS ещё на 17513, появился ECH |
| `chrome116` | `curl_chrome116` | hello 107-й с перемешиванием порядка расширений (110+), без ECH |
...
| `firefox147` | `curl_firefox147` (135 и 144 шлют тот же hello) | SCT (18) после delegated credentials |
| `safari153` | `curl_safari153` | 26 шифров вместо 20, без `compress_certificate` |
| `safari170` | `curl_safari170` | форма 15.5, но `SETTINGS_ENABLE_PUSH` впереди (`2:0;4:4194304;3:100`) и `Sec-Fetch-*` в запросе |
| `safari172ios` | `curl_safari172_ios` | то же с окном потока 2 МиБ и iOS-UA |
...
`standby-rezka.tv`: у гласящих GREASE форм — `IllegalParameter`, у `firefox147`,
`firefox147` и `tor145`, которые не гласят, — `OK`. `tls.peet.ws` подтвердил
...
* **Регистр имён заголовков в HTTP/1.1.** hyper умел писать исходный регистр с
  ...
  `Accept-Encoding`, `TE`, `Te` у `curl_firefox147` — у него обёртка однострочная,
...
* **Перемешивание расширений.** Chromium 110+ тасует порядок расширений каждое
  ...
  `curl_chrome123`, а захваты форка пишут `tls_permute_extensions: true` у всех
...
* **ECH в форме GREASE.** Все девять записей с `--ech true`
  (`chrome123/131/131android/133/136`, `firefox133`, `firefox135/144`, `tor145`)
...
* `captures` — `SAME` у двенадцати записей; у пяти хромов `ext order` (наша
  выборка из распределения против одной выборки в захвате), у `chrome123` ещё
...
расхождение `hello-diff` у `chrome123` «padding'ом, который бандл добавляет ниже
...
про порядок, а не про набор. Пропущенным оказалось другое: `chrome123` и
...
* `echo-diff` — `SAME` у тринадцати записей из восемнадцати, включая все четыре
  ...
  соединение. JA4 совпал у всех, `chrome123` и `chrome131android` в том числе.
* `hello-diff` — `SAME` у тринадцати, включая четыре строки с ECH (`firefox133`,
  `firefox147`, `firefox147`, `tor145`): тело сравнивается по объявленной длине,
  ...
  (`chrome123`: 497 против 561 в том прогоне).
* `captures` — `SAME` у двенадцати; у пяти хромов `ext order`, у `chrome123` ещё
...
`curl_firefox147` совпадают полностью, кроме версии в UA, — остаётся `firefox147`.
...
**Версию в имени несёт самая новая из совпадающих.** Правило применено и к парам,
где запись уже существовала: `chrome120` переименован в `chrome123` (идентичность
123-й, у которой `accept-encoding` уже с `zstd`), `firefox144` — в `firefox147` (у
147-й `accept-language` сменился с `q=0.5` на `q=0.9`). Замерено по обёрткам:
`curl_chrome119`/`120`/`123` дают один JA4, один akamai-отпечаток, одни кадры и
одни имена заголовков, а `curl_firefox135`/`144`/`147` совпадают и того больше,
поэтому имя записи должно быть той версией, которую шлёт трафик.

**Три новые записи (2026-09-18).** Из обёрток, чья форма не повторяла ни одну
существующую, добавлены `chrome116`, `safari170` и `safari172ios`:

* `chrome116` (`curl_chrome116`) — hello Chrome 107 с перемешиванием порядка
  расширений: единственная форма в наборе, где перемешивание есть, а ECH нет.
  JA3 такого клиента не повторяется, JA4 совпадает с 107-й (`…e5627efa2ab1`),
  преамбула h2 — 107-я.
* `safari170` (`curl_safari170`) — hello 15.5-й, но `SETTINGS` =
  `2:0;4:4194304;3:100` (push впереди, `settings_order: &[2, 4, 3]`) и семь
  заголовков запроса вместо четырёх: три `Sec-Fetch-*` и `accept-language:
  en-US,en;q=0.9` вместо `en-GB,en-US;q=0.9,en;q=0.8`. Именно эту пару раньше
  скрывал алиас `safari170 → safari155`.
* `safari172ios` (`curl_safari172_ios`) — то же с окном потока 2 МиБ и iOS-UA.

Замерено `tools/fingerprint all`: `headers-diff` — `SAME` у всех трёх (имена,
регистр и порядок: 13 заголовков у 116-й, по 8 у Safari), `hello` — 517 байт у
всех трёх против 517 у бандла, `hello-diff` — `SAME` у обеих Safari (16
расширений, байт в байт) и `extensions DIFF` у 116-й ровно из-за перемешивания
(размер совпадает, 517 против 517), `echo-diff` — пусто у Safari и только JA3 у
116-й. Пины JA4 в тестах: `chrome116` — `…e5627efa2ab1`, `safari170` и
`safari172ios` — `…14788d8d241b`.

**Имена.** Имя профиля — ровно один `code`, и у клиента в нём всегда стоит версия
(`chrome146`, `firefox147`, `tor145`); без версии остаётся только `rustls`, у
...
firefox147` — `SAME`; `--legend` печатает двадцать профилей без токенов
...
### Padding под 512 байт (2026-09-18, четвёртый проход)

**Зазор закрыт.** Слот padding теперь считает расширения, которые идут после
него: `ClientExtensions::encode` (`vendor/rustls/src/msgs/handshake.rs`) кодирует
хвост — ECH, его outer-компаньон, PSK — в отдельный буфер и учитывает его длину,
прежде чем посчитать `padding_to - (тело + 4 + хвост)`. Раньше слот мерил только
то, что успело записаться до него, а типизированное тело ECH дописывалось после,
поэтому hello уходил за порог на целое тело (803 байта против 517 у бандла).

**Данные.** У `chrome123` и `chrome131android` вернулось `padding_to: Some(512)`, а
их порядок стал `CHROME_PADDING_AND_ECH_EXT_ORDER` — список Chrome 99–107 со слотом
padding и дописанным ECH, как у BoringSSL (padding в таблице, ECH добавляется
снаружи). Обе записи паддят, когда тело ECH самое короткое (одно соединение из
четырёх), и не паддят при трёх других длинах — ровно как клиент. В записи
`chrome131android` вторая половина формы (`chrome131`) остаётся без padding: её
hello с гибридной группой всегда выше порога.

**Проверено.** `dump` по восемь прогонов на запись: `chrome123` и
`chrome131android` дают 512 при коротком теле (JA4
`t13d1517h2_8daaf6152771_b1ff8ab2d16f` — тот же, что у бандла в padding-захвате) и
526/558/590 при остальных (JA4 `t13d1516h2_…02713d6af862`); `chrome107` —
неподвижно 512 (`…e5627efa2ab1`). В тестах обе величины JA4 принимаются для этих
двух записей, а `grease_version_leads_supported_versions` для записей с ECH
требует «не ниже порога» вместо ровно 512. Полный `tools/fingerprint all`:
`headers-diff` — `SAME` у всех, `hello-diff` — `SAME` у всех, кроме тасующих,
у `chrome123`/`chrome131android` расходится только монетка padding'а (517 против
561 и 561 против 593). `PATCH.diff` перегенерирован (1357 строк) и проверен
`patch -p1` на чистой копии crates.io.
