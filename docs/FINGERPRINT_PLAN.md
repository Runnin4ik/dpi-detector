...
| имена заголовков в нижнем регистре | все, кроме `safari18`, `safari184ios`, `safari260`, `safari260ios`, `firefox144` | `http::HeaderName` всегда в нижнем регистре, а обёртка (и браузер, с которого её писали) шлёт `Host`, `User-Agent`, `Accept`, `Accept-Encoding`, `Sec-Fetch-*` в исходном регистре; в h2 этого не видно по RFC 9113 |
| `priority: u=0, i` шлём и в h1 | записи с этим заголовком | curl выбрасывает его на h1-соединении (запрос бандла на 18 байт короче), мы — нет; в h2 его шлют обе стороны |

Чем закрываются (если понадобится 100% соответствия):
