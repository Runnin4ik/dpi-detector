...
| ECH | самодельное тело ECH заставляло Google/Cloudflare отвечать `fatal alert: DecodeError`; путь rustls (`EchMode::Grease` через `net::hpke`) принят всеми четырьмя хостами свипа | GREASE-ECH в девяти записях, длина тела зафиксирована комментарием |
...
| долей ключа меньше | `firefox`×3 (2 против 3), `tor145` (1 против 3) | rustls отправляет долю первой группы списка плюс гибридную компоненту (`client/hs.rs`: `shares = [первая, компонента]`), а бандл — `--tls-key-shares-limit 3`; у `firefox` третья доля, P-256, есть и в захвате форка |
...
* `hello-diff` — у Firefox-семейства и у Tor остаётся один `body ech`
  (пересобираемое тело), у пяти хромов — `extensions DIFF; body ech`: набор
  отличается на padding, который бандл добавляет, когда его hello (с более
  коротким телом ECH) проседает ниже 512 байт.
...
* **настоящий ECH по конфигурации из DNS** — единственный незакрытый пункт.
  Он делал бы hello зависимым от *хоста*, тогда как все остальные поля записи —
  от клиента, и обёртка его не шлёт; для диагностики он ничего не даёт (цензор
  не отличает GREASE-ECH от настоящего по построению), а стоит DNS-запроса в
  классифицируемом пути. Работа оценена: `Hpke` уже есть, нужны разбор
  HTTPS/SVCB (`build_dns_query` принимает любой тип, неизвестные RR уже
  приезжают как `DnsRecord::Other`) и кэш конфигурации.
* **длина тела GREASE-ECH** — у нас 446 байт против 187–283 у curl: у него
  меньше внутренний hello, а длина этой части случайна с обеих сторон (у бандла
  она меняется от соединения к соединению). Из-за неё наши 1.3-only хромы не
  опускаются ниже 512-байтового порога padding, у которого бандл иногда
  оказывается, — поэтому у него счёт расширений изредка на единицу больше.
