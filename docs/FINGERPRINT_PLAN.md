...
| под 512 байт padding не отправляется | `chrome123`, `chrome131android` | у клиента с самым коротким телом GREASE-ECH hello опускается до 497 байт и он паддит его до 517; слот padding в записи перелетает — `padding_to` меряется до того, как rustls допишет типизированное тело ECH, — поэтому шлётся вариант без padding, который сам клиент шлёт в трёх соединениях из четырёх |
