...
* **Решено: удалить** `probe/tls.rs` — 196 строк, `probe_tls_domain`,
  `probe_tls_domain_default`, `is_suspicious_redirect`; сейчас живут только в
  реэкспорте `probe/mod.rs:12`. Знание не теряется: те же сигнатуры блок-страниц,
  но разложенные по регионам, уже есть в `RegionProfile::blockpage_signatures`
  (`profile/mod.rs:117-149`) — это и остаётся единственным домом для них, а плоский
  регионо-независимый список из `probe/tls.rs:23-33` только дублировал его.
...
* Заодно `TCP_NODELAY`: он есть в `net/tcp.rs:32`, `dns/doh.rs:79`, `dns/dot.rs:102`
  ...
  `probe/tls.rs:55`, `dns/socks.rs:110`. **Решено: ставить везде** — одним хелпером
  (`net::tcp::set_no_delay`), чтобы правило из `net/tcp.rs:22` («это требование, а не
  оптимизация») было верным для всех проб, а не для четырёх из десяти.
...
### P5 — `i18n` из ядра в бинарник (решено: выносим)
