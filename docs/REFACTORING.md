...
Документ описывает работу по структуре кодовой базы. Фазы P0–P8 **выполнены**
(их коммиты — в §2), P9 решено не делать; раздел §1 остаётся списком инвариантов,
а §4 — историей фаз: что именно менялось, чем проверялось и какие отклонения были
приняты сознательно. Каждая фаза задумывалась как один коммит, который можно
откатить целиком, не меняя поведения (кроме фаз, где смена поведения — сама цель).
...
Все фазы плана, кроме P9 (типизированный конфиг), выполнены. Один коммит — одна
фаза, кроме отмеченных исключений.
...
| `21bb1e8` | — | Записан этот план |
| `1aebf22` | P1.5 (часть), P4 (часть) | Удалён мёртвый `probe/tls.rs` (196 строк), `TCP_NODELAY` включается одним хелпером на всех дозвонах |
| `bdd9186` | **P1** | `net/version.rs` ушёл в `dpi-detector::update`; ядро перестало зависеть от `i18n` |
| `6337df1` | **P1.5** | Публичная поверхность ядра: 30 реэкспортов `dns/mod.rs` → 3, `RustlsConnector`/`DpiTlsConnector` понижены до `pub(crate)`, удалены `DpiProbeStream::{into_inner,get_ref,get_mut}`, `QTYPE_{NS,SOA,PTR,MX}`, `Language::{code,name,is_rtl}`, три мёртвых аксессора `profile`, включён `#![warn(unreachable_pub)]` |
| `50afd0e` | **P2** | `dns/availability.rs` (1146) → `probe/dns_avail.rs`; ядро потеряло рёбра `dns → probe` и `dns → classify` |
| `6ff72e5` | **P2.5** | `detail` стал enum'ом `classify::Detail` с `code()`; `classify/constants.rs` удалён, классификация больше не разбирает прозу, `i18n::detail_text` — исчерпывающий `match` |
| `81bd6bc` | **P2.6** | `--json` собирается структурами `src/json.rs`; `runner.rs` больше не знает `json!` |
| `8724e7e` | **P2.7** | `i18n/mod.rs` (1711) → `mod.rs` + `messages.rs` + `en/ru/zh/fa` |
| `750e185` | **P4 + P4.5 + P7** | `net/netinfo.rs` (988) → `http_client` + `public_ip` + `sysinfo/{os,bypass}`, Cymru уехал в `probe/cymru.rs` (разорван цикл `net → dns`); девять TLS-фабрик → `TlsProfile` + `create_tls_config`; паники ядра обоснованы или убраны |
| `c7660fb` | **P3** | `render.rs` (3013) и `menu.rs` (1532) → `tui/{backend,widgets,progress,input,screens}` + `views/*`; markup-слой удалён |
| `ce546e3` | **P6** | CI-гейт `.github/workflows/check.yml` (build + test + clippy на push и PR) |
| `0dc0fa7` | **P5** | `i18n` переехал в бинарник; `Messages` похудел с 213 полей до 206 (мёртвые строки и рантайм-тест покрытия удалены) |
| `ac53995` | **P8** | `selection_flags` → `TestSelection::parse` с именованными полями |

Отклонения от плана, зафиксированные сознательно:

* **P4, P4.5 и P7-core легли одним коммитом** (`750e185`): они связаны через
  `net/http_client.rs` и новый `TlsProfile`, по отдельности ни одна часть не
  собирается.
* **P4.5 не сократил файл**: `net/tls.rs` 273 → 296 строк. Сократилась поверхность
  (девять фабрик → одна), а два крипто-провайдера, insecure-верификатор и разбор
  версий остались на месте.
* **`profile::blockpage_signatures` удалён** в P1.5: после удаления `probe/tls.rs`
  его не читал никто, а список подписей остаётся в истории git до появления
  потребителя.
* **`Messages` похудел**: семь полей не читал никто (`banner_subtitle`, `stage`,
  `bytes`, `duration`, `bypass_tools`, `gateway`, `detail_write_timeout`), пять
  читал только тест покрытия. Сам тест снят: четыре литерала в четырёх языках и так
  не компилируются, если поле забыли.
* **P7 не сводится к нулю паник**: осталось 11 `unwrap`/`expect` вне тестов, у
  каждого написано, почему он невозможен (провайдер TLS, константный HTTP-запрос,
  таблица спецификаций отпечатков, `serde_yaml` на пустом маппинге, пример-утилита).
* **P4 разложился иначе, чем в плане**: вместо `sysinfo/{os,bypass}.rs` появились
  ещё `net/public_ip.rs` и `net/sysinfo/mod.rs`.
...
### P1 — `net/version.rs` уходит из ядра в бинарник ✅ (`bdd9186`)
...
### P1.5 — Публичная поверхность ядра ✅ (`6337df1`)
...
* **Сделано** (`1aebf22`): `probe/tls.rs` удалён целиком.
* Осталось — понизить до `pub(crate)` либо удалить: `net/version.rs:154` `version_badge`
...
### P2 — Тест 1 переезжает из `dns/` в `probe/` ✅ (`50afd0e`)
...
### P2.5 — `detail` из строк в тип ✅ (`6ff72e5`)
...
### P2.6 — Типизированный `--json` ✅ (`81bd6bc`)
...
### P2.7 — `i18n` по файлам языков ✅ (`8724e7e`, позже уехал в бинарник) 
...
### P3 — Разрезать `render.rs` и `menu.rs` ✅ (`c7660fb`)
...
### P4 — Разделить `net/netinfo.rs` ✅ (`750e185`)
...
* **Сделано** (`1aebf22`): `TCP_NODELAY` включается на всех дозвонах через
  `net::tcp::set_no_delay` — DoH, DoT, SOCKS5, GET в `netinfo`, три пути Telegram и
  `dial_tcp`, так что правило из `net/tcp.rs:22` («это требование, а не оптимизация»)
  верно для каждого дозвона, а не для четырёх из десяти.

### P4.5 — Свернуть TLS-фабрики ✅ (`750e185`)
...
### P5 — `i18n` из ядра в бинарник ✅ (`0dc0fa7`)

Решение принято. Порядок обязателен: после **P1** (иначе ядру всё равно нужен `i18n`
для `net/version.rs`) и после **P2.5/P2.6** (чтобы прозу деталей не пришлось переносить
дважды): сначала `detail` становится типом с `code()`, потом тексты уезжают в
`dpi-detector/src/i18n/`.

* Что переезжает: `Messages` (4 блока), `legend_sections*()`, `details.rs`,
  `fingerprint_label`, `fmt_speed`/`fmt_size`, `Messages::{config_warning, phase_text}`.
* Что остаётся в ядре: `PhaseId`/`ProgressBlock` (языконезависимые идентификаторы) и
  константы деталей; `i18n` в бинарнике импортирует их — направление зависимости
  становится односторонним.
* Вместе с переносом правятся правила проекта (§1 этого документа и README): точка
  входа для новой строки — крейт бинарника, а не ядро.
* Проверка: тесты паритета четырёх блоков (`test_messages_coverage`,
  `test_finglish_is_latin_only`, `i18n::details::tests`) переезжают вместе с модулем;
  живой прогон меню и `--legend` на двух языках (`--lang en`, `--lang fa`).
...
### P7 — Аудит `unwrap()`/`expect()` вне тестов ✅ (`750e185`, разбор ниже) 
...
### P8 — `TestSelection` вместо 9-бушного кортежа ✅ (`ac53995`)
...
### P9 — Типизированный конфиг (решено: не делаем)

Слой терпимости к «грязным» ручным `config.yml` (числа строкой, регистр и вложенность
ключей, неизвестные ключи как предупреждения) остаётся: он покрыт тестами, включая
паритет с `config.yml`, и удаление сэкономило бы ~300 строк ценой отказа принимать
конфиги, которые сегодня работают. Решение принято 2026-09-14: типизацию не делаем.
...
| `enum DiagnosticVerdict` вместо `DpiStatus` | `DpiStatus` уже enum; типизировать нужно было `detail` — сделано в P2.5 (`classify::Detail`) |
