...
Опции:
  ...
  -o, --output <PATH>          Путь к файлу отчёта
      ...
      --burst-tls <VERSION>    Версия TLS для теста 7: 1.2|1.3 [по умолчанию: 1.3]
      --burst-alpn <PROTOCOL>  ALPN для теста 7: h2 (предлагает h2 с откатом на http/1.1)|http/1.1 (только http/1.1) [по умолчанию: h2]
...
При выборе теста `7` открывается свой экран настроек: число одновременных рукопожатий, таймаут одного рукопожатия, **версия TLS** (`1.2`/`1.3`), **протокол HTTP в ALPN** (`h2` — как у браузеров, с откатом на `http/1.1`, или `http/1.1` — только он), домен (пусто — весь список, как в тесте `2`) и отпечатки (по умолчанию все). `Q`/`Esc` отменяет тест.
