...
pub(crate) fn messages() -> Messages {
    Messages {
        ...
        intercept_excluded: "检测器的流量被访问策略「{}」排除。\n\
                             测试期间请设置 POLICY_EXCLUDE=1，或把 POLICY_NAME 改成路由器不存在的策略名",
        intercept_excluded_unnamed: "检测器的流量被访问策略排除。\n\
                                     测试期间请设置 POLICY_EXCLUDE=1，或把 POLICY_NAME 改成路由器不存在的策略名",
        ...
        intercept_unchecked_route: "无法确定我们的流量从哪个接口出去，\n\
                                    因此无法检查接口覆盖情况。",
        intercept_after: "应用更改后，请重启 nfqws2 和 dpi-detector。",
        ...
        cfg_warn_skipped_rows: "{} 中有无效行，已跳过，其余保留",
        ...
        warn_unknown_flag_value: "警告: 未知的 {} '{}'，改用 {}",
        ...
        cli_iface: "探测使用的接口: 设备名或其地址; 默认由路由表决定 (--iface wg0, --iface 172.16.0.2)",
        iface_unknown: "未找到接口 '{}': --iface 接受设备名或其地址, 菜单会列出可选接口",
        cli_concurrency: "并行请求的并发上限",
        cli_domain: "要测试的指定域名（可重复指定: -d vk.com -d ya.ru）",
        cli_output: "保存报告的输出文件路径",
        cli_burst: "Fingerprint/西伯利亚封锁 (测试 6): 每主机连接数, 重叠发起 [1-100, 默认: 10]",
        cli_burst_timeout: "测试 6: 握手超时, 秒 [默认: 8]; 之后的请求等待 read_timeout",
        cli_burst_gap: "测试 6: 尝试启动之间的延迟, 毫秒 [0-1000, 默认: 20]; 0 表示整个轮次在同一瞬间发起",
        ...
        cli_burst_variant: "测试 6 变体: 对每个配置的 ClientHello 施加一处修改，使判定可与其仅差一个字段的形态对比 — -ext:ID, ext-body:ID:HEX、+ext:ID、groups:A,B,…、key-shares:A,B,…、ext-order:A,B,…（0x0a0a 为 GREASE 槽位）、padding:N、no-padding、sigalg-swap、+grease、+group:ID、alpn-reverse；`;` 可同时施加多项 [默认: 无]",
        ...
        domains_load_failed: "无法构建目标列表 {} — 没有目标的测试不会测得任何结果，因此中止",
