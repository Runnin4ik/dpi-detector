...
pub(crate) fn messages() -> Messages {
    Messages {
        ...
        burst_field_attempts: "Attempts per host",
        burst_field_timeout: "Timeout in seconds",
        burst_field_gap: "Delay before requests",
        burst_gap_value: "{}ms",
        ...
        burst_variant_note: "Shape variant: {}",
        ...
        cli_iface: "Interface the probes leave through: a device name or one of its addresses; the routing table picks by default (--iface wg0, --iface 172.16.0.2)",
        iface_unknown: "Interface '{}' not found: --iface takes a device name or one of its addresses, and the menu lists them",
        ...
        cli_burst: "Fingerprint stress (test 6): connections per host, overlapping [1-100, default: 10]",
        cli_burst_timeout: "Test 6: handshake timeout, seconds [default: 8]; the request that follows waits read_timeout",
        cli_burst_gap: "Test 6: delay between attempt starts, ms [0-1000, default: 20]; 0 fires the whole round at one instant",
        ...
        cli_burst_variant: "Test 6 variant: one edit to every profile's ClientHello, so a verdict can be read against the shape it differs from in one field — -ext:ID, +ext:ID, groups:A,B,…, key-shares:A,B,…, ext-order:A,B,… (0x0a0a is a GREASE slot), padding:N, no-padding, sigalg-swap, +grease, +group:ID, alpn-reverse [default: none]",
