...
pub(crate) fn messages() -> Messages {
    Messages {
        ...
        intercept_list_mode: "В профиле «{}» стоит фильтр по списку: {}\n\
                              если наша цель в него входит, часть проверок пойдёт через обход, а часть мимо,\n\
                              и результаты будут смешанными: на время теста поставьте MODE_ALL вместо\n\
                              MODE_AUTO или MODE_LIST в NFQWS_EXTRA_ARGS",
        intercept_list_mode_own: "В профиле «{}» стоит фильтр по списку: {}\n\
                                  если наша цель в него входит, часть проверок пойдёт через обход, а часть мимо,\n\
                                  и результаты будут смешанными: на время теста уберите этот фильтр из {}",
