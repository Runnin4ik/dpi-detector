...
pub(crate) fn messages() -> Messages {
    Messages {
        ...
        menu_interface_auto: "по умолчанию",
        ...
        burst_field_attempts: "Попыток на домен",
        burst_field_timeout: "Таймаут в секундах",
        burst_field_gap: "Задержка перед запросами",
        burst_gap_value: "{}мс",
        ...
        burst_variant_note: "Вариант формы: {}",
        ...
        legend_profiles_ja4: "JA4 — хеш собственного ClientHello профиля, нечувствительный к порядку расширений: это ключ, который может нести матчер отпечатков. Форма, чей hello может провалиться под 512-байтовый порог padding'а, отвечает двумя строками (с `padding` и без); JA3 не печатается, потому что у формы с перемешиванием порядка единого JA3 нет.",
        lang: Language::Ru,
        ...
        intercept_header: "Обнаружен включённый nfqws2, но есть проблемы:",
        intercept_excluded: "Трафик детектора исключён политикой доступа «{}».\n\
                             На время тестов поставьте POLICY_EXCLUDE=1 или укажите свой POLICY_NAME,\n\
                             которого нет среди политик роутера",
        intercept_excluded_unnamed: "Трафик детектора исключён политикой доступа.\n\
                                     На время тестов поставьте POLICY_EXCLUDE=1 или укажите свой POLICY_NAME,\n\
                                     которого нет среди политик роутера",
        ...
        intercept_list_recipe: "Каждый профиль, который берёт порты, тестируемые детектором, сужен списком hostlist/ipset,\n\
        ...
        intercept_unchecked_config: "Разрешённый конфиг пакета не читается, поэтому о его правилах ничего не известно.\n\
                                     Проверьте установку пакета.",
        intercept_unchecked_queue: "Пакет запущен, но очередь не привязана. Попробуйте перезапустить nfqws2 и dpi-detector.",
        intercept_unchecked_route: "Не удалось определить, каким интерфейсом уходит наш трафик,\n\
                                    поэтому покрытие по интерфейсу не проверить.",
        intercept_after: "После применения изменений перезапустите nfqws2 и dpi-detector.",
        ...
        cfg_warn_skipped_rows: "{}: недопустимые строки пропущены, остальные используются",
        ...
        warn_unknown_flag_value: "Предупреждение: неизвестное значение {} '{}', используется {}",
        ...
        cli_iface: "Интерфейс, через который идут проверки: имя устройства или его адрес; по умолчанию выбирает таблица маршрутов (--iface wg0, --iface 172.16.0.2)",
        iface_unknown: "Интерфейс '{}' не найден: --iface принимает имя устройства или его адрес, список печатает меню",
        ...
        cli_burst: "Fingerprint/Сибирская блокировка (тест 6): соединений на домен, внахлёст [1–100, по умолчанию: 10]",
        cli_burst_timeout: "Тест 6: таймаут рукопожатия, секунды [по умолчанию: 8]; запрос после него ждёт read_timeout",
        cli_burst_gap: "Тест 6: задержка между стартами попыток, мс [0–1000, по умолчанию: 20]; 0 — весь раунд стартует в один момент",
        ...
        cli_burst_variant: "Тест 6, вариант: одна правка ClientHello каждого профиля, чтобы вердикт читался против формы, отличающейся одним полем — -ext:ID, ext-body:ID:HEX, +ext:ID, groups:A,B,…, key-shares:A,B,…, ext-order:A,B,… (0x0a0a — слот GREASE), padding:N, no-padding, sigalg-swap, +grease, +group:ID, alpn-reverse; `;` применяет несколько сразу [по умолчанию: нет]",
        ...
        domains_load_failed: "Не удалось собрать список целей {} — прогон без целей ничего не измеряет, поэтому он останавливается",
        domains_list_empty: "{} (нет ни одного пригодного домена)",
        whitelist_load_failed: "Не удалось прочитать файл белого списка SNI {}: используется встроенный список",
        tcp16_load_failed: "Не удалось прочитать список --tcp16 {} — встроенные цели измерили бы не те хосты, поэтому прогон останавливается",
...
pub(crate) fn legend_sections() -> Vec<(&'static str, Vec<(&'static str, &'static str)>)> {
    vec![
        ...
        ("— TCP 16-20KB тест —", vec![
            ("16KB DROP", "Обрыв чтения внутри окна 14–36 KB; в деталях — READ TIMEOUT at N KB"),
            ("DETECTED", "Обрыв при отправке в том же окне 14–36 KB — тот же 16KB DROP, но для теста 16 KB, где мы отправляем, а не читаем"),
            ("OK", "Все 10 запросов (до 40 КБ) прошли без обрыва"),
        ]),
        ("— QUIC —", vec![
            ("OK", "QUIC-эндпоинт ответил на Initial: UDP-путь до 443 работает, HTTP/3-стек жив (ServerHello, либо Retry с верным integrity-тегом)"),
            ("CLOSED", "Эндпоинт ответил защищённым CONNECTION_CLOSE или stateless reset: путь работает, рукопожатие — нет; в деталях — код transport error"),
            ("VN", "Эндпоинт не поддерживает QUIC v1 и перечислил версии, которые умеет"),
            ("SPOOF", "Retry с неверным integrity-тегом: пакет пришёл не от эндпоинта — тег может посчитать только тот, кто видел Initial"),
            ("DROP", "Ответа на Initial нет: UDP 443 фильтруется или эндпоинт недоступен (тишина, как SYN DROP для TCP)"),
            ("REFUSED", "ICMP: на UDP-порту никто не слушает — путь работает, QUIC-слушателя у эндпоинта нет вовсе; это не блокировка (блокировка — DROP)"),
        ]),
        ("— Прочее —", vec![
