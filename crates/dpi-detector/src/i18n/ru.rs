...
pub(crate) fn messages() -> Messages {
    Messages {
        ...
        intercept_excluded: "Соединение исключено политикой доступа «{}»\n\
                             временно поставьте POLICY_EXCLUDE=1 или укажите свой POLICY_NAME,\n\
                             которого нет среди политик роутера",
        intercept_excluded_unnamed: "Соединение исключено политикой доступа\n\
                                     временно поставьте POLICY_EXCLUDE=1 или укажите свой POLICY_NAME,\n\
                                     которого нет среди политик роутера",
        intercept_interface: "Правила висят на интерфейсе {}, а наш трафик уходит через {}\n\
                              добавьте этот интерфейс в ISP_INTERFACE в /opt/etc/nfqws2/nfqws2.conf\n\
                              (несколько — через пробел)",
        intercept_ports: "Наш порт он не обрабатывает\n\
                          добавьте порт в TCP_PORTS и в --filter-tcp стратегии",
        intercept_ipv6: "Детектор запущен в режиме IPv6, но правила для IPv6 не ставятся:\n\
                         в конфиге пакета IPV6_ENABLED=0\n\
                         поставьте IPV6_ENABLED=1 в /opt/etc/nfqws2/nfqws2.conf",
        intercept_tunnel: "Наш трафик уходит через туннель (VPN), а правила висят на интерфейсе провайдера\n\
                           трафик внутри туннеля обход не увидит: уберите эту цель из туннеля\n\
                           или добавьте интерфейс туннеля в ISP_INTERFACE",
        intercept_list_mode: "В профиле «{}» стоит фильтр по списку: {}\n\
                              это список из режима работы, поэтому часть проверок пойдёт через обход,\n\
                              а часть мимо: на время теста поставьте MODE_ALL вместо MODE_AUTO или MODE_LIST\n\
                              в NFQWS_EXTRA_ARGS",
        intercept_list_mode_own: "В профиле «{}» стоит фильтр по списку: {}\n\
                                  часть проверок пойдёт через обход, часть мимо — результаты будут смешанными:\n\
                                  на время теста уберите этот фильтр из {}",
        ...
        intercept_unchecked_config: "Разрешённый конфиг пакета не читается, поэтому о его правилах ничего не известно\n\
                                     проверьте установку пакета и перезапустите службу",
        intercept_unchecked_queue: "Пакет запущен, но очередь не привязана — служба не успела подняться\n\
                                    перезапустите её",
