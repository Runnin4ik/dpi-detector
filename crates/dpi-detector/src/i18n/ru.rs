...
pub(crate) fn messages() -> Messages {
    Messages {
        ...
        intercept_excluded: "Обнаружен включённый nfqws2, но трафик детектора он не видит:\n\
                             соединение исключено политикой доступа «{}».\n\n\
        ...
        intercept_excluded_unnamed: "Обнаружен включённый nfqws2, но трафик детектора он не видит:\n\
                                     соединение исключено политикой доступа.\n\n\
                                     Чтобы обход применялся и к самому роутеру, временно поставьте POLICY_EXCLUDE=1\n\
                                     или укажите свой POLICY_NAME, которого нет среди политик роутера, затем:",
        intercept_interface: "Обнаружен включённый nfqws2, но трафик детектора он не видит:\n\
                              правила висят на интерфейсе {}, а наш трафик уходит через {}.\n\n\
                              Чтобы обход применялся и к нашему трафику, добавьте этот интерфейс\n\
                              в ISP_INTERFACE в /opt/etc/nfqws2/nfqws2.conf (несколько — через пробел), затем:",
        intercept_ports: "Обнаружен включённый nfqws2, но трафик детектора он не видит:\n\
                          наш порт он не обрабатывает.\n\n\
                          Добавьте порт в TCP_PORTS и в --filter-tcp стратегии, затем:",
        intercept_ipv6: "Обнаружен включённый nfqws2, но трафик детектора он не видит:\n\
                         правила для IPv6 не ставятся — в конфиге пакета IPV6_ENABLED=0.\n\n\
                         Чтобы обход работал и по IPv6, поставьте IPV6_ENABLED=1\n\
                         в /opt/etc/nfqws2/nfqws2.conf, затем:",
        intercept_tunnel: "Обнаружен включённый nfqws2, но трафик детектора он не видит:\n\
                           наш трафик уходит через туннель (VPN), а правила висят на интерфейсе провайдера.\n\n\
                           Трафик внутри туннеля обход не увидит: уберите эту цель из туннеля\n\
                           или добавьте интерфейс туннеля в ISP_INTERFACE, затем:",
        intercept_list_mode: "Обнаружен включённый nfqws2, но стратегия работает не для всех целей:\n\
                              в профиле «{}» стоит фильтр по списку: {}\n\n\
                              Это список из режима работы, поэтому часть проверок пойдёт через обход, а часть мимо.\n\
                              На время теста поставьте MODE_ALL вместо MODE_AUTO или MODE_LIST в NFQWS_EXTRA_ARGS, затем:",
        intercept_list_mode_own: "Обнаружен включённый nfqws2, но стратегия работает не для всех целей:\n\
                                   в профиле «{}» стоит фильтр по списку: {}\n\n\
                                   Часть проверок пойдёт через обход, часть мимо — результаты будут смешанными.\n\
                                   На время теста уберите этот фильтр из профиля в NFQWS_ARGS_CUSTOM, затем:",
        intercept_unchecked: "Обнаружен включённый nfqws2, но очередь не отвечает — проверка не удалась.\n\
                              Похоже, служба не успела подняться, перезапустите её:",
