...
/// Written out because the two sinks are `Arc<dyn Fn …>`, which is not `Debug`:
/// what a caller can print is that the struct is there and how many sinks it
/// carries, not the closures behind them.
impl std::fmt::Debug for PhaseProgress {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PhaseProgress").finish_non_exhaustive()
    }
}
